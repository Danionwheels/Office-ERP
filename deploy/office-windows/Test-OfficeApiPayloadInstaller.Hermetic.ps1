...
$checks = [ordered]@{
    ...
    enumeratesPackage = $source.Contains('Get-ChildItem -LiteralPath $packagePath -Force') -and $source.Contains('Copy-Item -LiteralPath $_.FullName')
    ...
    powershell51HashCompatibility = $source.Contains('ComputeHash') -and $source.Contains('BitConverter') -and -not $source.Contains('SHA256]::HashData') -and -not $source.Contains('Convert]::ToHexString')
