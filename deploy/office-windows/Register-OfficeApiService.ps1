...
$databaseSecretsDirectory = Join-Path $ProgramDataRoot 'Secrets\Database'
$applicationPassfilePath = Join-Path $databaseSecretsDirectory 'application.pgpass'
...
if (-not (Test-Path -LiteralPath $databaseSecretsDirectory -PathType Container) -or -not (Test-Path -LiteralPath $applicationPassfilePath -PathType Leaf)) { throw 'The protected application PostgreSQL passfile is required before API service registration.' }
...
if ($null -ne $existing) {
    ...
    $actualPath = ([string]$existing.PathName).Trim()
    $pathMatches = $actualPath -eq $expectedPath -or
        $actualPath -eq ('"' + $expectedPath + '"') -or
        $actualPath.StartsWith('"' + $expectedPath + '" ', [StringComparison]::OrdinalIgnoreCase) -or
        $actualPath.StartsWith($expectedPath + ' ', [StringComparison]::OrdinalIgnoreCase)
    if (-not $pathMatches) { throw "A foreign service already owns '$ServiceName'." }
...
function Set-OfficeMachineSecretAcl([string]$SecretsRoot, [string]$Directory, [string]$Envelope) {
    ...
    $secretsSecurity = [Security.AccessControl.DirectorySecurity]::new()
    $secretsSecurity.SetOwner($systemSid)
    $secretsSecurity.SetAccessRuleProtection($true, $false)
    foreach ($identity in @($systemSid, $administratorsSid)) {
        [void]$secretsSecurity.AddAccessRule([Security.AccessControl.FileSystemAccessRule]::new(
            $identity,
    ...
    [void]$secretsSecurity.AddAccessRule([Security.AccessControl.FileSystemAccessRule]::new(
        ...
        [Security.AccessControl.PropagationFlags]::None,
        [Security.AccessControl.AccessControlType]::Allow))
    Set-Acl -LiteralPath $SecretsRoot -AclObject $secretsSecurity
...
function Set-OfficeApplicationPassfileAcl([string]$Directory, [string]$Passfile) {
    $systemSid = [Security.Principal.SecurityIdentifier]::new('S-1-5-18')
    $administratorsSid = [Security.Principal.SecurityIdentifier]::new('S-1-5-32-544')
    $serviceSid = [Security.Principal.SecurityIdentifier]::new('S-1-5-80-2177609957-237951300-3651597395-3114367455-1078186923')
    ...
    [void]$fileSecurity.AddAccessRule([Security.AccessControl.FileSystemAccessRule]::new(
        ...
        [Security.AccessControl.PropagationFlags]::None,
        [Security.AccessControl.AccessControlType]::Allow))
    Set-Acl -LiteralPath $Passfile -AclObject $fileSecurity
}
...
if ($PSCmdlet.ShouldProcess($ServiceName, 'Register or verify demand-start API service')) {
    ...
    Set-OfficeMachineSecretAcl $secretsRoot $machineSecretDirectory $machineSecretPath
    Set-OfficeApplicationPassfileAcl $databaseSecretsDirectory $applicationPassfilePath
