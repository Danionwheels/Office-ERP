...
| `SEC04-02C2` | `CURRENT` | Implement the EF operator repository, including serialized Administrator mutations. | `SEC04-02C1` | Focused repository tests prove tracked aggregate reads/writes, normalized lookup, ambiguity reads, active-admin queries, and the PostgreSQL transaction lock contract. |
| `SEC04-02C3` | `PENDING` | Generate the Auth-schema migration and prove migration/model parity against PostgreSQL. | `SEC04-02C2` | Upgrade from the latest existing migration succeeds, repository integration tests pass, and EF reports no pending model changes. |
...
| `SEC04-04` | `PENDING` | Make login/session authorization read active persisted operators instead of packaged `IOptions` users. | `SEC04-02C3`, `SEC04-03` | Existing auth tests plus permission/status-change invalidation pass. |
