...
| `SEC04-02C3` | `CURRENT` | Generate the Auth-schema migration and prove migration/model parity against PostgreSQL. | `SEC04-02C2` | Upgrade from the latest existing migration succeeds; repository integration proves reads/writes, normalized lookup, active-admin queries and advisory locking; EF reports no pending model changes. |
