...
| `SEC04-05` | `DONE` | Remove packaged Development operators and session secrets from non-Development settings. | `SEC04-04` | Base/published settings contain no operator identity or signing secret; non-Development configuration users are rejected, missing Production signing material fails closed, and the disposable staging contract no longer advertises ignored operator variables. API 95/95, staging preflight 20/20, and Compose validation pass. |
| `SEC04-06A` | `DONE` | Implement versioned DPAPI envelopes with atomic replacement, lifecycle mutex, tamper detection, and non-secret fingerprints. | `SEC04-01` | Versioned LocalMachine DPAPI store uses a global lifecycle mutex, same-directory write-through temporary files, atomic replacement with rollback, post-write decrypt/parse verification, authenticated payload validation, disposable key snapshots, and SHA-256 ciphertext evidence. Nine focused tests cover preservation, replacement, both interruption boundaries, tamper, concurrency, redaction, and real Windows DPAPI; API 104/104 and the 26-project solution build pass with zero warnings/errors. |
| `SEC04-06B` | `DONE` | Converge machine-secret directory/file ACLs and expose secrets only through the installed configuration provider. | `SEC04-06A` | Commit `e2f82036` plus smoke-harness fix `56500e87`; native normal-account denial and all CI release gates green in run [29740746818](https://github.com/Danionwheels/Office-ERP/actions/runs/29740746818). |
...
| `SVC05-02` | `DONE` | Define API installed paths, virtual service identity, receipts, ownership checks, and preservation rules. | `SVC05-01` | Accepted [office-api-service-contract-v1.md](../architecture/office-api-service-contract-v1.md); service identity, ACL, collision, and preservation rules are frozen. |
| `SVC05-03` | `DONE` | Atomically install and verify the API/UI payload under `%ProgramFiles%`. | `SVC05-02` | Installer and 8-check hermetic contract pass in hosted CI run [29776011994](https://github.com/Danionwheels/Office-ERP/actions/runs/29776011994). |
| `SVC05-04` | `DONE` | Register the API service in demand-start mode and apply least-privilege binary, configuration, log, and pgpass ACLs. | `SVC05-03`, `SEC04-06B` | Service identity, dependency, collision refusal, recovery, and 9-check ACL contract pass in hosted CI run [29776011994](https://github.com/Danionwheels/Office-ERP/actions/runs/29776011994). |
| `SVC05-05A` | `DONE` | Generate installed non-secret Production configuration and bind protected values through the machine-secret provider. | `SVC05-04` | Generator, provider fail-closed validation, and hosted Windows run [29776011994](https://github.com/Danionwheels/Office-ERP/actions/runs/29776011994) pass; contract frozen in [office-production-configuration-contract-v1.md](../architecture/office-production-configuration-contract-v1.md). |
| `SVC05-05B` | `DONE` | Configure the PostgreSQL service dependency, delayed automatic start, and bounded recovery actions. | `SVC05-05A` | Native PostgreSQL lifecycle and service activation contracts pass in hosted CI run [29778495916](https://github.com/Danionwheels/Office-ERP/actions/runs/29778495916). |
| `SVC05-06A` | `DONE` | Activate the API service only after database, migrations, first operator, and `/ready` succeed. | `SVC05-05B`, `SEC04-07A` | Readiness-gated activation contract passes locally and in hosted CI run [29778495916](https://github.com/Danionwheels/Office-ERP/actions/runs/29778495916); failed prerequisite/readiness stops the service with an actionable reason. |
| `SVC05-06B` | `DONE` | Add narrowly classified API-service repair operations and ownership checks. | `SVC05-06A` | Repair contract passes locally and in hosted CI run [29778495916](https://github.com/Danionwheels/Office-ERP/actions/runs/29778495916); missing/owned/foreign states are classified, and mutation requires explicit `-Apply`. |
| `SVC05-07` | `DONE` | Add the readiness-aware launcher with operator-friendly recovery guidance. | `SVC05-06B` | Launcher contract passes locally and in hosted CI run [29778495916](https://github.com/Danionwheels/Office-ERP/actions/runs/29778495916); it opens only `127.0.0.1:5188` after database readiness without requiring a terminal. |
| `SVC05-08` | `DONE` | Add owned Start-menu entry and optional desktop shortcut with idempotent repair. | `SVC05-07` | Shortcut contract passes locally and in hosted CI run [29778495916](https://github.com/Danionwheels/Office-ERP/actions/runs/29778495916); exact launcher target/arguments and repair behavior are fixed. |
| `SVC05-09` | `DONE` | Implement API/shortcut/binary uninstall that preserves all office data and protected state. | `SVC05-04`, `SVC05-08` | Uninstall contract passes locally and in hosted CI run [29778495916](https://github.com/Danionwheels/Office-ERP/actions/runs/29778495916); foreign or receipt-less services are refused and PostgreSQL/operator/secrets/log state is preserved. |
| `SVC05-10A` | `DONE` | Compose one elevated setup entry for preflight, VC++, PostgreSQL, and migrations. | `SVC05-01`, `DB03-04`, `SVC05-02` | One-entry setup contract passes locally and in hosted CI run [29778495916](https://github.com/Danionwheels/Office-ERP/actions/runs/29778495916); explicit roots and the `DatabaseReady` checkpoint are fixed. |
...
| `SVC05-10C` | `DONE` | Extend the same setup entry through API service, launcher/shortcut, and final readiness. | `SVC05-03`, `SVC05-05B`, `SVC05-06B`, `SVC05-08`, `SVC05-10B` | One invocation reaches routine operator entry with explicit rollback boundaries; hosted Windows release gate [29794401181](https://github.com/Danionwheels/Office-ERP/actions/runs/29794401181) passed. |
| `SVC05-11A` | `DONE` | Add the complete one-setup lifecycle to the hermetic Windows gate. | `SVC05-10C`, `SVC05-09` | Interruption, recovery, ACL drift, and reinstall simulations pass in hosted Windows release gate [29794401181](https://github.com/Danionwheels/Office-ERP/actions/runs/29794401181). |
| `SVC05-11B` | `CURRENT` | Run the complete setup/service lifecycle on a disposable native Windows host. | `SVC05-11A` | Native-host acceptance runbook and 9-check contract are prepared in [office-native-setup-acceptance-v1.md](../architecture/office-native-setup-acceptance-v1.md); physical disposable-host evidence is now the next required action. |
