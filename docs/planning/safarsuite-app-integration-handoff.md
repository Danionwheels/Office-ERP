...
The remaining app-side gap is no longer the module-gateway contract, first image wrapper, image push, tag discipline, local container proof, control-side manifest diagnostics, app health/log/profile evidence, or a stub-cloud app-runtime install rehearsal; it is the real Control Cloud customer setup package pass with diagnostics upload, app activation import, rerun-secret proof, and later operational deployment-profile behavior beyond the commercial control channel.
