...
- App-to-LocalServer pairing implementation beyond the local authority lifecycle slice: signed local pairing key custody, Control Cloud/Control Desk first-manager setup-token issuance/download, real manager-session protection for device approval endpoints, final signed device credential verification, Windows app discovery UI, protected pairing-profile storage, and no-repeat client reconnect behavior; design captured in `docs/planning/safarsuite-app-local-server-pairing-flow.md`.
