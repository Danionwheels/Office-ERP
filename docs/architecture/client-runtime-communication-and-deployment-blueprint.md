...
- App-to-LocalServer pairing implementation beyond the local authority lifecycle slice: signed local pairing key custody, Windows app discovery UI, protected pairing-profile storage, no-repeat client reconnect behavior, and Client Portal pairing visibility; design captured in `docs/planning/safarsuite-app-local-server-pairing-flow.md`.
