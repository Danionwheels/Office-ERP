...
Control Cloud invitation management now accepts scoped provider bearer sessions and requires the `client-portal:manage` scope before creating, listing, resending, or revoking invitations. Provider sessions can be minted from configured provider-operator credentials through `POST /api/v1/provider-access/operator-sessions`; the shared-secret session endpoint and legacy `X-SafarSuite-Provider-Key` header remain as compatibility bootstrap paths while durable provider-user administration, MFA, and password reset are built.
