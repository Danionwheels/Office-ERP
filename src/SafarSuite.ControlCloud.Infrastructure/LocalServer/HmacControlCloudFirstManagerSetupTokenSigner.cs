...
public sealed class HmacControlCloudFirstManagerSetupTokenSigner
    : IControlCloudFirstManagerSetupTokenSigner
{
    ...
    public HmacControlCloudFirstManagerSetupTokenSigner(
        ControlCloudEntitlementSigningOptions options)
    {
        ...
        if (string.IsNullOrWhiteSpace(_activeKeyId))
        {
            throw new InvalidOperationException(
                "ControlCloud:EntitlementSigning:ActiveKeyId is required for first-manager setup-token signing.");
        }

        if (!_secretsByKeyId.ContainsKey(_activeKeyId))
        {
            throw new InvalidOperationException(
                $"Control Cloud first-manager setup-token signing key '{_activeKeyId}' is not configured.");
        }
    }

    public string SigningKeyId => _activeKeyId;

    public LocalServerSignedFirstManagerSetupTokenResponse Sign(
